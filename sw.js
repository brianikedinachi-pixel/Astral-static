...
async function _maybeShowConvoNotif() {
  ...
  const bodies = [
    "I'm still here, listening whenever you're ready.",
    "Still with you — tap to keep talking.",
    "Whenever you're ready, I'm right here.",
    "Still on the line. Tap to come back to our conversation.",
  ];
  const body = bodies[(_notifCount - 1) % bodies.length];

  try {
    await self.registration.showNotification('🟦 Astral · Talking with you', {
      body: elapsed > 0 ? `${body}  ·  ${elapsed}m` : body,
