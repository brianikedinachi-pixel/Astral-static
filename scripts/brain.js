...
// ── Master TTS — Server proxy → Web Speech ────────────────────────────────────
...
async function _masterTTS(text, onDone, opts = {}) {
  ...
  // Engine 1: server (Gemini 3.1 → 2.5 → Google Translate internally)
  const result = await _serverTTS(text, sig);
  if (result === 'aborted') return;

  if (!result) {
    // Engine 2: Web Speech — always available, device-quality neural voice
    _webSpeechTTS(text, onDone, opts);
    return;
  }
...
async function speakMessage(idx, btn) {
    ...
    try {
      ...
      if (result && result !== '_timeout_' && result.url) {
        ...
        await audio.play().catch(() => {
          // Autoplay blocked or other error: use Web Speech immediately
          window._ttsAudioEl = null;
          speak(log.aiText, onDone, true);
        });
        return;
  ...
  // ── Path 2: No cache yet — speak via Web Speech immediately (zero delay)
  //            AND simultaneously fire a server fetch for future cache.
  //            The user hears something instantly; next tap plays quality audio.
  speak(log.aiText, onDone, true);
  // Start background fetch for next time (don't re-add if already in map)
...
function _stopConvoAudio() { _masterTTSStop(); }

const _CONVO_THINKING_SOUNDS = [
  "Hmm...", "Let me think...", "Give me a sec...", "Right, so...",
  "I hear you...", "Mmm, got it...", "Okay, one sec...", "Yeah, sure...",
  "Let me see...", "Mm-hmm...", "Alright...", "Interesting...",
];

function _convoThinkFiller() {
  const r = _CONVO_THINKING_SOUNDS[Math.floor(Math.random() * _CONVO_THINKING_SOUNDS.length)];
  _webSpeechTTS(r, null, { rate: 1.0, pitch: 0.95, forConvo: true });
}
...
/* ── Main convo TTS — word-by-word bubble + real audio ─────────────────────── */
function _speakConvo(text, isFiller) {
  if (isFiller) {
    _webSpeechTTS(text, null, { rate: 1.0, pitch: 0.95, forConvo: true });
    return;
  }
  ...
  // Build word list for bubble streaming
  const words = text.replace(/\s+/g, ' ').trim().split(' ');
  let wordIdx = 0;

  // Near-silent utterance purely for word boundary events
  const u = new SpeechSynthesisUtterance(text);
  u.volume = 0.01;
  u.rate   = 1.0;
  u.onboundary = (ev) => {
    if (ev.name !== 'word') return;
    // Advance bubble words up to current char position
    let chars = 0, w = 0;
    for (const word of words) {
      if (chars >= ev.charIndex) break;
      chars += word.length + 1;
      w++;
    }
    while (wordIdx < w && wordIdx < words.length) {
  ...
  u.onend = () => {
    // Flush any remaining words (Safari fires fewer boundary events)
    while (wordIdx < words.length) { _convoAppendWord(words[wordIdx]); wordIdx++; }
  };

  // Pulse orb while audio loads from server
  let fallbackPulse = setInterval(_triggerWordPulse, 240);
  const stopPulse   = () => { if (fallbackPulse) { clearInterval(fallbackPulse); fallbackPulse = null; } };

  speechSynthesis.speak(u);

  // Real quality audio via master TTS (Gemini → Web Speech fallback)
  _masterTTS(text, () => {
    stopPulse();
    speechSynthesis.cancel();
    // Flush all words to bubble
    while (wordIdx < words.length) { _convoAppendWord(words[wordIdx]); wordIdx++; }
...
async function _convoSendToAI(userText) {
  ...
  _convoHistory.push({ role: 'user', text: userText });
  htmlResult.push({ humanText: userText, aiText: '', thinking: false, likes: 0, dislikes: 0 });
  const chat = getCurrentChat();
  if (chat) { chat.messages = htmlResult; chat.updatedAt = new Date().toISOString(); }
  renderMessages();
  ...
  try {
    ...
    if (e.name === 'AbortError') {
      _speakConvo("Sorry, that took too long. Could you say it again?", false);
    } else {
      _speakConvo("Sorry, something went wrong on my end. Say that again?", false);
    }
  }
}
...
function _convoStartListening() {
  ...
  r.onerror = (ev) => {
    ...
    _stopEmotionCapture();
    if (_convoMode && !_convoMuted && !_convoThinking) _scheduleConvoRestart(800);
  };

  r.onend = () => {
    _convoRecognition = null;
    if (_convoMode && !_convoThinking && !_convoMuted) _scheduleConvoRestart(300);
  };

  try { r.start(); } catch(e) {
    _convoRecognition = null;
    _stopEmotionCapture();
    if (_convoMode && !_convoThinking && !_convoMuted) _scheduleConvoRestart(1000);
  }
}
