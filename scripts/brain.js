...
async function _convoThinkFiller() {
  ...
  try {
    ...
    const _wasListening = !!_alwaysOnRec;
    if (_alwaysOnRec) {
    ...
    const cleanup = () => {
      ...
      if (window._fillerAudioEl === audio) window._fillerAudioEl = null;
      // Resume listening once the filler clip is done, but only if we're
      // still mid-turn and nothing else (TTS) has already taken over the mic.
      if (_wasListening && _convoMode && !_convoMuted && !_convoTTSPlaying && _convoThinking) {
        _scheduleConvoRestart(150);
      }
    };
...
async function _speakConvo(text) {
  ...
  const _finish = () => {
    ...
    if (_convoMode && !_convoMuted && !_convoThinking) {
      // If the user spoke again while we were thinking/speaking, that speech
      // was queued (see _commitAon) instead of dropped — send it now rather
      // than sitting idle in "Listening…" while ignoring what was already said.
      if (_convoQueuedUserText) {
        const queued = _convoQueuedUserText;
        _convoQueuedUserText = '';
        _scheduleConvoRestart(700); // still bring the mic back for the *next* turn
        _convoSendToAI(queued);
        return;
      }
...
function _commitAon() {
  ...
  // If still thinking from a previous turn, don't drop this — the user spoke
  // while waiting for a reply (very common with the thinking filler playing).
  // Queue it; it gets sent the moment the current turn finishes instead of
  // being silently lost (which is what produced the "Hmm, I missed that"
  // loop, since the *next* thing the mic picked up was effectively ignored).
  if (_convoThinking) {
    _convoQueuedUserText = (_convoQueuedUserText + ' ' + text).trim();
    return;
  }
