...
async function startConvoMode() {
  // Bypass for owner account — check every possible source
  let _bypassEmail = '';
  try {
    _bypassEmail = session?.email
      || JSON.parse(localStorage.getItem('astral_session') || '{}').email
      || '';
  } catch(e) {}
  const _isBypassUser = _bypassEmail === 'bukanwoko@gmail.com';
  ...
  const SR = window.SpeechRecognition || window.webkitSpeechRecognition;
  if (!SR) {
    alert('Voice conversation requires Chrome, Edge, or Safari.');
    return;
  }
