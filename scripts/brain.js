...
async function checkTopUserStatus() {
  ...
  // Local quick-check: if this user has sent messages, attempt the backend check
  // (userProfile.messageCount is tracked locally)
  var localCount = (userProfile && userProfile.messageCount) || 0;

  try {
    var r = await fetch(SERVER_URL + '/admin-stats?admin_email=check_top&user_email=' + encodeURIComponent(session.email));
    if (!r.ok) {
      console.warn('Top user check failed, status:', r.status);
      return;
    }
    var d = await r.json();
    console.log('Top user check result:', d);
    if (d.is_top_user) {
      showTopUserToast(d.top_message_count || localCount);
    }
  } catch(e) {
    console.warn('Top user check error:', e);
  }
}

function showTopUserToast(count) {
  if (document.getElementById('top-user-toast')) return;
  ...
  if (!document.getElementById('top-toast-style')) {
    ...
    styleEl.textContent = [
      ...
      '@media(max-width:480px){',
        '#top-user-toast{font-size:0.78rem;padding:11px 14px;gap:9px;border-radius:16px;}',
        '#top-user-toast .t-icon{width:26px;height:26px;font-size:0.88rem;}',
      '}',
    ].join('');
