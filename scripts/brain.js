...
function _buildAlwaysOnRec() {
  ...
  r.onresult = (ev) => {
    ...
    // Rebuild the full transcript from scratch every event instead of
    // incrementally appending across events. Some mobile speech engines
    // (notably Android Chrome) occasionally redeliver a "final" result
    // whose transcript is the *entire phrase recognised so far* rather
    // than just the newly-finalised word — appending that onto text we
    // already had produced runaway duplication ("oh oh did oh did I oh
    // did I say…"). Reconstructing from ev.results each time is immune
    // to however the engine chooses to redeliver/revise segments.
    let finalText = '';
    let interimText = '';
    for (let i = 0; i < ev.results.length; i++) {
    ...
    const gotFinal = finalText && finalText !== _aonPendingFinal.trim();
    _aonPendingFinal  = finalText ? finalText + ' ' : _aonPendingFinal;
