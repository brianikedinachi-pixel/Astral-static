    ...
    if (log.thinking || (log.aiText !== undefined && log.aiText !== '')) {
      ...
      if (log.thinking) {
        aiPart += '<div class="thinking-log" id="thinking-log-' + idx + '">' +
          '<div class="tl-row"><span class="tl-arc"><span></span></span><span class="tl-text" id="tl-text-' + idx + '"></span></div>' +
        '</div>';
      } else {
        // AI content
...
async function handleSend() {
  ...
  try {
    ...
    if (data.file_card) {
      ...
      // Smart filename: reject generic backend names, derive from user message instead
      var _badName = /^(astral[_\-\s]?output|converted|output|file|untitled)/i;
      var _fcName  = (fc.filename && !_badName.test(fc.filename))
        ? fc.filename
      ...
      // Push card into live htmlResult so renderMessages sees it
      var _fcEntry = { humanText: '', aiText: '', _fileCard: { fileId: fileId, filename: _fcName, fmt: fc.ext } };
      htmlResult.push(_fcEntry);
      var _fcChat = getCurrentChat();
      if (_fcChat) { _fcChat.messages = htmlResult; }
      saveToStorage();
      // No auto-download — user opens the card and clicks Download themselves
    }
    // ────────────────────────────────────────────────────────────────────────
