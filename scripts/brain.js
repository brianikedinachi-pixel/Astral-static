...
/* ── Load chats: localStorage FIRST (instant), then sync backend in background ── */
async function loadChatHistoryFromBackend() {
  // Step 1: Load localStorage immediately so chats appear instantly
  ...
  // Step 2: Sync backend in the background (non-blocking — won't delay UI)
  if (session?.email) {
    (async () => {
      try {
          ...
          if (found && found.text) {
            try {
              ...
              if (Array.isArray(parsed) && parsed.length > 0) {
                ...
                // Refresh sidebar with synced data
...
async function init() {
  ...
  // Load localStorage instantly; backend syncs in background
