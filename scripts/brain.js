...
// Initial check: navigator.onLine can be wrong when page loads from SW cache.
// Confirm with a real network request before showing the banner.
(async function checkInitialConnectivity() {
  if (navigator.onLine) return; // trust positive values
  try {
    // cdn.jsdelivr.net is excluded from SW cache so this hits the real network
    const ctrl = new AbortController();
    setTimeout(() => ctrl.abort(), 3000);
    await fetch('https://cdn.jsdelivr.net/npm/marked/package.json', {
      method: 'HEAD', cache: 'no-store', signal: ctrl.signal, mode: 'no-cors'
    });
    // Fetch succeeded — navigator.onLine was wrong, stay online
  } catch {
    // Both agree: actually offline
