...
async function _speakConvo(text) {
  ...
  const _finish = () => {
    ...
    if (_convoMode && !_convoMuted) {
      ...
      setTimeout(_convoStartListening, 300);
    }
  };
...
function _convoStartListening() {
  if (!_convoMode || _convoThinking || _convoMuted || _convoSpeaking) return;
  ...
  // Tear down any existing recognition instance cleanly
  ...
  let _gotResult = false; // guard: prevent onend from double-scheduling after a valid result
  ...
  r.onerror = (ev) => {
    ...
    if (ev.error === 'not-allowed' || ev.error === 'service-not-allowed') {
      // Mic permission revoked — stop gracefully
      stopConvoMode();
      return;
    }
    _stopEmotionCapture();
  ...
  r.onend = () => {
    _convoRecognition = null;
    if (_gotResult || !_convoMode || _convoThinking || _convoMuted || _convoSpeaking) return;
    // Normal end-of-utterance: restart listening right away (small delay avoids rapid spin)
    _scheduleConvoRestart(_convoMicFailCount > 0 ? _convoMicRetryDelay() : 200);
  };
...
async function startConvoMode() {
  ...
  if (!_isPWA) {
    ...
    if (!_CONVO_VIP_EMAILS.includes(_currentEmail)) {
      _showConvoError(
        ...
        'Tap the "get app" button at the top to install it — it\'s instant!'
      );
      // Also flash the install chip so they know where to look
      const chip = document.getElementById('install-chip');
      if (chip) { chip.style.display = 'inline-block'; chip.style.animation = 'none'; chip.style.boxShadow = '0 0 0 3px rgba(0,234,255,0.5)'; setTimeout(() => { chip.style.boxShadow = ''; }, 2000); }
      return;
    }
  }
  ...
  _backendTTSAvailable  = null; // reset so TTS availability is re-probed this session
...
document.addEventListener('visibilitychange', () => {
  ...
  if (document.hidden) {
    ...
    if (_convoListenTimer) { clearTimeout(_convoListenTimer); _convoListenTimer = null; }
    _stopEmotionCapture();
    _minimizeConvo(); // shrink to the top-right bubble now, so there's no flash of the big orb on return
    _registerConvoNotification();
