...
function _setConvoState(state) {
  ...
  if (status) {
    status.textContent =
      ...
      state === 'muted'     ? 'Muted — tap mic to resume' :
      'Listening…';
  }
}
...
function _triggerWordPulse() {
  ...
  if (mini) { mini.classList.remove('word-pulse'); void mini.offsetWidth; mini.classList.add('word-pulse'); }
}
...
async function _convoSendToAI(userText) {
  ...
  try {
    ...
    if (_convoMode) setTimeout(_startEmotionCapture, 300);
