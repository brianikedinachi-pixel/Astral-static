...
window.convertImageToDoc = async function() {
  ...
  try {
    ...
    // ZIP/DOCX → immediate download; others → inject card then show preview
    if (DIRECT_DOWNLOAD_FMTS.has(fmt)) {
      _triggerDownload(blob, filename);
      _injectFileCard(fileId, filename, fmt);
    } else {
      _injectFileCard(fileId, filename, fmt);
      // Auto-open preview for text files
...
var _STAGE_LABELS = {
  ...
  'filegen':   '📄 Generating file',
  'memory':    '🧠 Recalling memory',
  'thinking':  '✨ Thinking',
  'reply':     '✏️ Composing reply',
...
function _startActivitySSE() {
  ...
  _activitySSE.onmessage = function(e) {
    ...
    try {
      ...
      var label = _STAGE_LABELS[data.stage] || ('● ' + data.stage);
      var snippet = data.msg ? (': ' + data.msg.slice(0, 55) + (data.msg.length > 55 ? '…' : '')) : '';
...
function renderMessages() {
  ...
  displayContainer.innerHTML = htmlResult.map(function(log, idx) {
    ...
    if (log._fileCard) {
      ...
      return '<div class="chatlog" data-idx="' + idx + '">' +
        '<div class="file-card" onclick="openFilePreview(\'' + fc.fileId + '\')" title="' + hint + '">' +
    ...
    if (log.thinking || (log.aiText !== undefined && log.aiText !== '')) {
      ...
      if (log.thinking) {
        aiPart += '<div class="thinking-log" id="thinking-log-' + idx + '">' +
          '<div class="tl-row">' +
            '<span class="tl-orb-wrap"><span class="tl-orb"></span><span class="tl-orb"></span><span class="tl-orb"></span></span>' +
            '<span class="tl-text" id="tl-text-' + idx + '">Thinking…</span>' +
          '</div>' +
        '</div>';
      } else {
        // AI content
...
async function handleSend() {
  ...
  try {
    ...
    if (data.file_card) {
      ...
      var isBin   = DIRECT_DOWNLOAD_FMTS.has(fc.ext);

      // Decode base64 → Blob
      var byteChars = atob(fc.data_b64);
      var byteArr   = new Uint8Array(byteChars.length);
      for (var i = 0; i < byteChars.length; i++) byteArr[i] = byteChars.charCodeAt(i);
      var blob = new Blob([byteArr], { type: fc.mime });
      ...
      var chat2 = getCurrentChat();
      if (chat2) {
        chat2.messages.push({ humanText: '', aiText: '', _fileCard: { fileId: fileId, filename: fc.filename, fmt: fc.ext } });
        saveToStorage();
      }

      // ZIP/DOCX straight download; text files auto-open preview
      if (isBin) {
        _triggerDownload(blob, fc.filename);
      }
    }
    // ────────────────────────────────────────────────────────────────────────
