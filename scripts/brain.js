...
function _getAudioCtx() {
  try {
    ...
    console.warn('Web Audio context unavailable:', e);
    return null;
  }
}
...
function _playAudioBlob(blob, onDone, signal) {
  return new Promise((resolve) => {
    ...
    // Only apply if Web Audio context is available; avoid cracking from double-processing
    try {
      const ctx  = _getAudioCtx();
      if (ctx && ctx.state !== 'closed') {
        try {
          const src  = ctx.createMediaElementSource(audio);
          const low  = ctx.createBiquadFilter(); low.type = 'peaking'; low.frequency.value = 200; low.gain.value = 2.5;
          const hi   = ctx.createBiquadFilter(); hi.type  = 'peaking'; hi.frequency.value = 4800; hi.gain.value = 1.2;
          const comp = ctx.createDynamicsCompressor();
          comp.threshold.value = -25; comp.ratio.value = 3; comp.attack.value = 0.005; comp.release.value = 0.2;
          src.connect(low); low.connect(hi); hi.connect(comp); comp.connect(ctx.destination);
        } catch(audioErr) {
          console.warn('Web Audio processing skipped:', audioErr);
        }
      }
  ...
  displayContainer.innerHTML = htmlResult.map(function(log, idx) {
    ...
    if (log.humanText || log.humanImage) {
      humanPart = '<div class="human-response">' +
        ...
        (log.humanText  ? '<p>' + escHtml(log.humanText) + '</p>' : '') +
        '</div>';
    }

    var aiPart = '';
    if (log.thinking || (log.aiText !== undefined && log.aiText !== '') || log._fileCard) {
      aiPart = '<div class="ai-response" style="font-weight:normal;font-style:normal;letter-spacing:normal;line-height:1.5;">';
      if (log.thinking) {
        ...
        // AI content
        if (log.aiText) {
          aiPart += '<div class="ai-content" style="font-weight:normal;font-style:normal;letter-spacing:normal;line-height:1.5;">' + renderMarkdown(log.aiText) + '</div>';
...
async function _convoThinkFiller() {
  ...
  try {
    ...
    // ── Pause the always-on mic while the filler plays ──────────────────────
    // Same crackle issue as full TTS: holding SpeechRecognition open while
    // audio plays forces the OS into a "voice call" audio route on mobile,
    // which makes the filler clip crack/distort. Suspend the mic for the
    // ~1s clip and resume right after — short enough that barge-in loss is
    // not noticeable, and it kills the crackle entirely.
    _stopAlwaysOnMic();

    // Use the shared processed audio pipeline to prevent crackling
    await _playAudioBlob(blob, null, null);
    // Keep mic OFF while thinking - do NOT resume here
    // We only resume listening in _finish() when AI is completely done talking.
  } catch(e) {}
}
...
async function _speakConvo(text) {
  ...
  // Stop the mic immediately when starting to speak
  _stopAlwaysOnMic();
  ...
  const _finish = () => {
    ...
    // AI finished talking - resume listening immediately
    if (_convoMode && !_convoMuted && !_convoThinking) {
      _setConvoState('listening');
      // Restart the mic now that the AI is done talking
      _scheduleConvoRestart(100); 
    }
  };
  ...
  await new Promise((resolve) => {
    ...
    // Use the shared processed audio pipeline to prevent crackling
    _playAudioBlob(blob, cleanup, sig).then(resolve);

    // Start word ticker immediately (approximation for convo mode)
...
function _convoAddBubble(role, text) {
  ...
  if (role === 'user') {
    ...
    htmlResult.push({ humanText: '', aiText: '', thinking: false, likes: 0, dislikes: 0, _convoMode: true });
...
async function _convoSendToAI(userText) {
  ...
  try {
    ...
    const data  = await resp.json();
    const reply = (data.reply || '').trim() || "I'm sorry, I didn't quite catch that. Could you repeat it?";
...
let _convoQueuedUserText = ''; // speech captured while a previous turn was still "thinking"
...
function _commitAon() {
  ...
  // If AI is speaking or thinking, we should have already stopped the mic.
  // But as a safety guard, if we're here, we process it.
  if (_convoSpeaking || _convoThinking) {
    // In "no-listen-while-talking" mode, we don't barge in or queue.
    // We just ignore it or stop the mic if it's somehow still on.
    _stopAlwaysOnMic();
    return;
  }

  // Stop the mic immediately before processing the request
  _stopAlwaysOnMic();
  _convoSendToAI(text);
}
...
function _stopAlwaysOnMic() {
  if (_alwaysOnRec) {
    try {
      _alwaysOnRec.onend = null;
      _alwaysOnRec.onerror = null;
      _alwaysOnRec.abort();
    } catch (e) {}
    _alwaysOnRec = null;
  }
