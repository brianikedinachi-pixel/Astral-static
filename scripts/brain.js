...
// ── Shared /tts JSON fetch helper ──────────────────────────────────────────────
// The server returns JSON now — { audio (base64), format, engine, spoken_text,
// word_timings } — not a raw audio blob, so every caller decodes through here.
// spoken_text/word_timings are what make Convo Mode's captions track the real
// voice exactly instead of guessing: spoken_text is the literal text that was
// actually synthesised (after the server's own markdown/contraction cleanup),
// and word_timings — when present — is genuine per-word timing (native from
// Edge TTS, or forced-aligned against the generated audio for every other
// engine), not an estimate.
...
async function _serverTTS(text, signal) {
  try {
    ...
    const result = await _fetchTTSJson({ text, lang: 'en', slow: false }, signal);
...
function _prefetchTTSAudio(idx, rawText) {
  ...
  const promise = (async () => {
    try {
      const result = await _fetchTTSJson({ text, lang: 'en', slow: false });
...
function _fetchTTSChunk(text) {
  ...
  return (async () => {
    try {
      return await _fetchTTSJson({ text, lang: 'en', slow: false });
    } catch(e) { return null; }
  })();
}
...
async function _getFillerAudioBlob(phrase) {
  ...
  const promise = (async () => {
    ...
    try {
      const result = await _fetchTTSJson({ text: phrase, lang: 'en', slow: false });
      if (!result) return null;
...
function _setConvoState(state) {
  ...
  if (wrap) {
    wrap.classList.remove('state-connecting', 'state-listening', 'state-thinking', 'state-speaking', 'state-analyzing', 'state-muted');
  ...
  if (mini) {
    mini.classList.remove('state-connecting', 'state-listening', 'state-thinking', 'state-speaking', 'state-analyzing', 'state-muted');
