...
function _startActivitySSE() {
  ...
  _activitySSE.onmessage = function(e) {
    ...
    try {
      ...
      if (!label) return;  // not a heavy stage — ignore
...
function _startActivity() {
  ...
  _setInlineLog('');  // silent spinner until a heavy stage fires
  _startActivitySSE();
    ...
    if (log.thinking || (log.aiText !== undefined && log.aiText !== '')) {
      ...
      if (log.thinking) {
        aiPart += '<div class="thinking-log" id="thinking-log-' + idx + '">' +
          '<div class="tl-row"><span class="tl-spinner"></span><span class="tl-text" id="tl-text-' + idx + '">Working…</span></div>' +
        '</div>';
      } else {
        // AI content
