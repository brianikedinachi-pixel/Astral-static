    ...
    if (log.thinking || (log.aiText !== undefined && log.aiText !== '')) {
      ...
      if (log.thinking) {
        aiPart += '<div class="thinking-log" id="thinking-log-' + idx + '">' +
          '<div class="tl-row"><span class="tl-arc"><span></span></span><span class="tl-text" id="tl-text-' + idx + '" style="min-width:0"></span></div>' +
        '</div>';
      } else {
        // AI content
...
async function handleSend() {
  ...
  try {
    ...
    if (data.file_card) {
      ...
      // Decode base64 → Blob (chunked for speed — avoids stalling on large files)
      var blob = (function(b64, mime) {
