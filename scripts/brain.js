...
async function startConvoMode() {
  ...
  if (!_isPWA) {
    ...
    if (!_CONVO_VIP_EMAILS.includes(_currentEmail)) {
      ...
      // — give a concise browser-specific nudge
      const _isIOS = /iphone|ipad|ipod/i.test(navigator.userAgent);
      const _msg = _isIOS
        ? 'To use conversation mode, install Astral: tap the Share icon in Safari, then "Add to Home Screen".'
        : 'To use conversation mode, install Astral: open your browser menu and choose "Install app" or "Add to Home Screen".';
      _showConvoError(_msg);
      _convoStarting = false; return;
    }
  }
