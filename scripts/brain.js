...
async function _speakConvo(text) {
  ...
  // Start barge-in mic immediately so user can cut in at any time
  _startBargeInListening();
  ...
  const _finish = () => {
    ...
    _convoSpeaking = false;
    _stopBargeInListening(); // no-op with always-on mic
...
function _buildAlwaysOnRec() {
  ...
  r.onend = () => {
    ...
    _alwaysOnRec = null;
    // Restart immediately — always-on means always-on
