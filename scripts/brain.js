...
function _startEmotionCapture() {
  if (_convoSpeaking) return; // never compete with TTS playback for audio focus
  _emotionAudioChunks = [];
...
async function _speakConvo(text) {
  ...
  await new Promise((resolve) => {
    let wordTicker = null;
    const WORDS_PER_SEC = 3.2; // natural English speech rate
    ...
    // Start word ticker as soon as play() resolves (before 'playing' fires on mobile).
    const startTicker = () => {
    ...
    audio.addEventListener('playing', startTicker, { once: true });
    audio.addEventListener('canplay', startTicker, { once: true }); // fires earlier on Android
...
async function _convoSendToAI(userText) {
  ...
  try {
    ...
    // _convoStartListening already calls _startEmotionCapture when listening begins.
...
let _aonCommittedIdx      = 0;      // ev.results index already consumed — never reprocess below this
...
function _buildAlwaysOnRec() {
  ...
  r.onstart = () => {
    ...
    _aonCommittedIdx   = 0; // brand-new session — its results array starts empty
    _setConvoState(_convoSpeaking ? 'speaking' : 'listening');
    // IMPORTANT: never open a second getUserMedia stream while the AI is
    // speaking — on Android Chrome this steals audio focus from the TTS
    // <audio> element and cuts the voice off mid-sentence. Only capture
    // for emotion analysis once we're actually listening to the user, and
    // never stack a second recorder on top of one that's already running.
    if (!_convoSpeaking && (!_emotionRecorder || _emotionRecorder.state === 'inactive')) {
  ...
  r.onresult = (ev) => {
    ...
    // In continuous mode, ev.results keeps growing for the *entire* session —
    // it is never cleared between utterances, only when a brand-new
    // recognition object starts (handled by resetting _aonCommittedIdx in
    // onstart above). Rebuilding from ev.results[0] every event (as a
    // previous version of this code did) meant that once a session stayed
    // alive across multiple turns, every new word caused the ENTIRE
    // conversation-so-far to be re-sent, which looked like the mic had
    // stopped responding to new speech. Tracking a committed index — and
    // never looking at entries we've already consumed — fixes that.
    // We also take only the LATEST final entry in a given unseen batch
    // (not a sum of all of them), since some mobile engines redeliver a
    // "final" result as the whole phrase recognised so far rather than
    // just the newly-finalised words, which caused the earlier "oh oh did
    // oh did I…" duplication bug.
    let newFinal = '';
    let interimText = '';
    for (let i = _aonCommittedIdx; i < ev.results.length; i++) {
    ...
    if (newFinal) {
      _aonCommittedIdx = ev.results.length; // never reprocess anything up to here
      _aonPendingFinal += newFinal + ' ';
      if (_aonSilenceTimer) clearTimeout(_aonSilenceTimer);
      _aonSilenceTimer = setTimeout(_commitAon, 1200);
    }
    _aonInterimLatest = interimText;
    _aonUpdateStatus();
    if (newFinal) _convoMicFailCount = 0;
  };
...
function _startAonWatchdog() {
  ...
  _aonWatchdog = setInterval(() => {
    ...
    if (Date.now() - _aonLastResultTime > 18000) {
      // Stale — poke it
      _aonLastResultTime = Date.now();
      if (_alwaysOnRec) {
        try { _alwaysOnRec.onend = null; _alwaysOnRec.onerror = null; _alwaysOnRec.abort(); } catch(e) {}
...
async function startConvoMode() {
  ...
  // ── "Get the app" gate — browser (non-PWA) users only; VIPs always pass ──────
  ...
  const _isVIP = _CONVO_VIP_EMAILS.includes(_currentEmail);
  if (!_isPWA && !_isVIP) {
    _showConvoError(
      "Conversation mode works best in the Astral app. " +
      "Tap the ‘get app’ button to install it — it's instant!"
    );
    return;
  }
