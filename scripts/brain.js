...
function _pickBestVoice() {
  ...
  // Tier 1 — Neural/Online voices (most human-sounding, require network)
  const tier1 = [
    'Microsoft Aria Online (Natural)',   // Edge/Windows — top tier
    'Microsoft Jenny Online (Natural)',
    'Microsoft Guy Online (Natural)',
    'Microsoft Ana Online (Natural)',
    'Google UK English Female',          // Android/Chrome neural
    'Google US English',
  ];
  // Tier 2 — High-quality local voices
  const tier2 = [
    'Samantha',   // macOS/iOS — best local
    'Karen',      // Australian — very natural
    'Moira',      // Irish — warm
    'Daniel',     // British male — clear
    'Serena',     // macOS UK female
    'Fiona',      // Scottish — distinct
  ];
  ...
  // Fallback — any English voice, prefer ones labelled 'Natural' or 'Online'
  const natural = voices.find(v => v.lang.startsWith('en') && (v.name.includes('Natural') || v.name.includes('Online') || v.name.includes('Neural')));
...
function speak(text, onDone, forceSpeak) {
  if (!forceSpeak && !voiceEnabled || !text) { if(onDone) onDone(); return; }
  speechSynthesis.cancel();
  const cleaned = _prepareTextForSpeech(text);
  if (!cleaned) { if(onDone) onDone(); return; }
  const u = new SpeechSynthesisUtterance(cleaned);
  // Natural speech settings
  u.rate   = 1.0;    // natural pace — not rushed
  u.pitch  = 0.97;   // slightly lower — warmer, less robotic
  u.volume = 1;
...
function speakMessage(idx, btn) {
  ...
  // Always speak when user manually taps the speak button (forceSpeak=true)
  speak(log.aiText, () => {
...
let _currentAudio = null; // tracks playing Audio element for interruption

// Stop any in-progress TTS (Puter audio or browser fallback)
...
// ── Puter.js loader ──────────────────────────────────────────────────────────
// Puter.js gives free access to Gemini TTS, xAI TTS, and AWS Polly neural
// voices — no API keys needed, no login required for these providers.
let _puterReady = false;
let _puterFailed = false;
let _puterQueue = [];

function _loadPuter(cb) {
  if (_puterReady)  { cb(false); return; }
  if (_puterFailed) { cb(true);  return; }
  _puterQueue.push(cb);
  if (_puterQueue.length > 1) return; // already loading
  const s = document.createElement('script');
  s.src = 'https://js.puter.com/v2/';
  s.onload = () => {
    // Give Puter 800ms to initialise before first call
    setTimeout(() => {
      _puterReady = true;
      _puterQueue.forEach(fn => fn(false));
      _puterQueue = [];
    }, 800);
  };
  s.onerror = () => {
    _puterFailed = true;
    _puterQueue.forEach(fn => fn(true));
    _puterQueue = [];
  };
  document.head.appendChild(s);
}

// Play an HTMLAudioElement returned by Puter and call onDone when finished
function _playPuterAudio(audio, text, onDone, nextFn) {
  if (!audio) { nextFn(text, onDone); return; }
  _currentAudio = audio;
  audio.onended = () => { _currentAudio = null; if (onDone) onDone(); };
  audio.onerror = () => { _currentAudio = null; nextFn(text, onDone); };
  try { audio.play(); } catch(e) { _currentAudio = null; nextFn(text, onDone); }
}

// ── Provider 1: Gemini TTS (Google's best neural voices, 30 options) ─────────
// Uses gemini-2.5-flash-preview-tts — sounds very human, emotionally aware.
// Voices: Aoede (warm), Charon (calm), Fenrir (confident), Kore (clear), Puck (upbeat)
const _GEMINI_VOICE = 'Aoede'; // warm, natural female
const _GEMINI_MODEL = 'gemini-2.5-flash-preview-tts';

async function _speakGemini(text, onDone) {
  return new Promise(resolve => {
    _loadPuter(async (err) => {
      if (err || typeof puter === 'undefined') { resolve(false); return; }
      try {
        const audio = await puter.ai.txt2speech(text, {
          provider: 'gemini',
          model: _GEMINI_MODEL,
          voice: _GEMINI_VOICE,
          instructions: 'Speak naturally and warmly, like a caring friend in conversation.'
        });
        _playPuterAudio(audio, text, onDone, _speakXAI);
        resolve(true);
...
// ── Provider 2: xAI (Grok) TTS — expressive, conversational ─────────────────
// Voices: ara (warm), eve (energetic), sal (smooth), rex (confident), leo (authoritative)
async function _speakXAI(text, onDone) {
  _loadPuter(async (err) => {
    if (err || typeof puter === 'undefined') { _speakPolly(text, onDone); return; }
    try {
      const audio = await puter.ai.txt2speech(text, {
        provider: 'xai',
        voice: 'ara', // warm voice
        output_format: 'mp3'
      });
      _playPuterAudio(audio, text, onDone, _speakPolly);
    } catch(e) { _speakPolly(text, onDone); }
  });
}

// ── Provider 3: AWS Polly neural — reliable, always-on ───────────────────────
// Uses Joanna (neural) — crisp, natural American English female voice.
async function _speakPolly(text, onDone) {
  _loadPuter(async (err) => {
    if (err || typeof puter === 'undefined') { _speakBrowserTTS(text, onDone); return; }
    try {
      const audio = await puter.ai.txt2speech(text, {
        voice: 'Joanna',
        engine: 'neural',
        language: 'en-US'
      }); // default provider is aws polly
      _playPuterAudio(audio, text, onDone, _speakBrowserTTS);
    } catch(e) { _speakBrowserTTS(text, onDone); }
  });
}

// ── Provider 4: Browser Web Speech — always available, no network needed ─────
function _speakBrowserTTS(text, onDone) {
  ...
  u.pitch = 0.95;
  u.volume = 1;
  const applyVoice = () => { const v = _pickBestVoice ? _pickBestVoice() : null; if (v) u.voice = v; };
  u.onend   = () => { if (onDone) onDone(); };
  u.onerror = () => { if (onDone) onDone(); };
  u.onboundary = (ev) => { if (ev.name === 'word') _triggerWordPulse(); };
  if (!speechSynthesis.getVoices().length) {
    speechSynthesis.addEventListener('voiceschanged', function once() {
      speechSynthesis.removeEventListener('voiceschanged', once);
      applyVoice(); speechSynthesis.speak(u);
    }, { once: true });
  } else { applyVoice(); speechSynthesis.speak(u); }
}

// ── Main convo voice entry point ─────────────────────────────────────────────
// Cascade: Gemini → xAI → Polly → Browser. First success wins, silent fallback.
async function _speakConvoVoice(text, onDone) {
  const ok = await _speakGemini(text, onDone);
  if (!ok) _speakXAI(text, onDone); // Gemini unavailable — try xAI
}

// Thinking fillers — natural, conversational, varied
const _CONVO_THINKING_SOUNDS = [
  "Hmm...", "Let me think...", "Interesting...", "Give me a sec...",
  "Hmm, okay...", "Right, so...", "I hear you...", "Mmm, got it...",
  "Okay okay...", "Sure, let me think about that...", "Hmm, that's a good one...",
  "Alright...", "Yeah, one sec...", "Let me see...", "Good question...",
  "Mm-hmm, thinking...", "Okay, so...", "Right right...",
];

function _convoThinkFiller() {
  const r = _CONVO_THINKING_SOUNDS[Math.floor(Math.random() * _CONVO_THINKING_SOUNDS.length)];
  // Always use browser TTS for fillers — instant, no network delay
  _speakBrowserTTS(r, null);
}
...
function _setConvoState(state) {
  ...
  if (status) {
    status.textContent = state === 'thinking' ? 'Thinking…' : state === 'speaking' ? 'Speaking…' : 'Listening…';
  }
}
...
// Main convo TTS entry point
function _speakConvo(text, isFiller) {
  if (isFiller) {
    _speakBrowserTTS(text, null);
    return;
  }
  ...
  // Pulse orb while audio loads
  let fallbackPulse = setInterval(_triggerWordPulse, 260);
  const stopPulse = () => { clearInterval(fallbackPulse); fallbackPulse = null; };

  // Allow barge-in: start listening in background while AI speaks
  setTimeout(() => {
    if (_convoMode && !_convoThinking) _convoStartListening();
  }, 600);

  _speakConvoVoice(text, () => {
    stopPulse();
...
async function _convoSendToAI(userText) {
  // If AI is speaking, interrupt it immediately (user barged in)
  if (_currentAudio || window._isSpeaking) {
    _stopConvoAudio();
    speechSynthesis.cancel();
  }
  // If still processing a prior request, ignore the new one
...
function _convoStartListening() {
  // Allow listening even while AI is speaking (barge-in support)
  ...
  r.onresult = (ev) => {
    ...
    for (let i = 0; i < ev.results.length; i++) {
      if (ev.results[i].isFinal) {
        ...
        interim += ev.results[i][0].transcript;
        // Show interim even while AI is speaking (barge-in feedback)
        const status = document.getElementById('convo-status-text');
        if (status) status.textContent = '(interrupting) ' + interim;
      }
    }
    const status = document.getElementById('convo-status-text');
    if (status && final_) status.textContent = '';
    if (final_.trim()) {
...
function stopConvoMode() {
  ...
  _convoMuted     = false;
  _convoWasHidden = false;
  _stopConvoAudio(); // stops ElevenLabs Audio + browser TTS
      ...
      <button id="convo-orb-btn" class="convo-orb-btn" type="button" aria-label="Conversation active — tap to stop">
        <svg class="convo-orb-wave-svg" viewBox="0 0 64 64" xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="none">
          ...
          <g clip-path="url(#orb-clip)">
            <path class="convo-orb-wave convo-orb-wave--1" fill="var(--wave-col-1, rgba(0,220,255,0.5))"
              d="M-16,38 C0,22 16,52 32,38 C48,22 64,52 80,38 L80,80 L-16,80 Z">
              <animate attributeName="d" dur="3.2s" repeatCount="indefinite"
                values="M-16,38 C0,22 16,52 32,38 C48,22 64,52 80,38 L80,80 L-16,80 Z;
                        M-16,44 C0,30 16,58 32,44 C48,30 64,58 80,44 L80,80 L-16,80 Z;
                        M-16,38 C0,22 16,52 32,38 C48,22 64,52 80,38 L80,80 L-16,80 Z"/>
            </path>
            <path class="convo-orb-wave convo-orb-wave--2" fill="var(--wave-col-2, rgba(0,160,200,0.35))"
              d="M-16,46 C8,34 24,58 40,46 C56,34 72,56 80,46 L80,80 L-16,80 Z">
              <animate attributeName="d" dur="2.6s" repeatCount="indefinite"
                values="M-16,46 C8,34 24,58 40,46 C56,34 72,56 80,46 L80,80 L-16,80 Z;
                        M-16,40 C8,54 24,34 40,50 C56,62 72,40 80,50 L80,80 L-16,80 Z;
                        M-16,46 C8,34 24,58 40,46 C56,34 72,56 80,46 L80,80 L-16,80 Z"/>
            </path>
          </g>
        </svg>
      </button>
...
document.addEventListener('visibilitychange', () => {
  ...
  if (document.hidden) {
    ...
    // Keep convo alive — just push a live notification so user sees it
    _registerConvoNotification();
  } else if (_convoWasHidden) {
    // Returned to app — cancel notification, show mini bubble
    _convoWasHidden = false;
    _unregisterConvoNotification();
    _minimizeConvo();
  }
