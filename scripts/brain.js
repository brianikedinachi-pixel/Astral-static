...
/* ══════════════════════════════════════════════════════
   THOUGHT LOG — SSE connection to /stream-log
   Shows live backend stages while Astral is thinking.
   Faint overlay text that disappears on reply.
══════════════════════════════════════════════════════ */
var _sseSource       = null;   // EventSource handle
var _logVisible      = true;   // user preference (can hide)
var _logMessages     = [];     // ring buffer, max 6 shown
var _logDismissTimer = null;

// Stage → human-friendly label
var STAGE_LABELS = {
  boot:   '🌐 Booting',
...
function ensureLogOverlay() {
  if (document.getElementById('astral-log-overlay')) return;
  ...
  style.textContent = [
    ...
    '}',
    '#astral-log-overlay.hidden{opacity:0;}',
    '.astral-log-line{',
      'font-family:"DM Sans","Segoe UI",sans-serif;',
      'font-size:0.72rem;',
      'color:rgba(0,220,255,0.38);',  /* very faint */
      'letter-spacing:0.05em;',
      'white-space:nowrap;overflow:hidden;text-overflow:ellipsis;',
      'max-width:100%;',
      'animation:logFadeIn .25s ease forwards;',
    '}',
    '.astral-log-line.old{color:rgba(0,220,255,0.18);}',
    '.astral-log-line.latest{color:rgba(0,220,255,0.62);font-weight:500;}',
    ...
    '#astral-log-toggle{',
      ...
      'border-radius:8px;',
      'padding:4px 8px;',
      'font-size:0.6rem;color:rgba(0,220,255,0.45);',
      'cursor:pointer;pointer-events:all;',
      'font-family:"DM Sans",sans-serif;letter-spacing:0.06em;',
      'transition:color .2s,border-color .2s;',
      'backdrop-filter:blur(4px);',
    '}',
    '#astral-log-toggle:hover{color:rgba(0,220,255,0.8);border-color:rgba(0,220,255,0.4);}',
    /* mobile: move toggle above input bar */
    '@media(max-width:600px){',
      '#astral-log-overlay{bottom:80px;}',
      '#astral-log-toggle{bottom:80px;right:10px;}',
    '}',
  ].join('');
  document.head.appendChild(style);

  var overlay = document.createElement('div');
  overlay.id = 'astral-log-overlay';
  document.body.appendChild(overlay);

  var toggle = document.createElement('button');
  toggle.id = 'astral-log-toggle';
  toggle.textContent = 'hide log';
  toggle.addEventListener('click', function() {
    _logVisible = !_logVisible;
    var el = document.getElementById('astral-log-overlay');
    if (el) el.classList.toggle('hidden', !_logVisible);
    toggle.textContent = _logVisible ? 'hide log' : 'show log';
  });
...
function _renderLogLines() {
  var el = document.getElementById('astral-log-overlay');
  if (!el) return;
  var last4 = _logMessages.slice(-4);
  el.innerHTML = last4.map(function(m, i) {
    var cls = 'astral-log-line';
    if (i === last4.length - 1) cls += ' latest';
    else if (i < last4.length - 2) cls += ' old';
    return '<div class="' + cls + '">' + _stageLabel(m.stage) + ' — ' + escHtml(m.msg) + '</div>';
  }).join('');
}
...
function startThoughtStream() {
  ...
  try {
    ...
    _sseSource.onerror = function() {
      // silently stop — the overlay will just show last known state
      if (_sseSource) { try { _sseSource.close(); } catch(ex){} _sseSource = null; }
    };
  } catch(e) {
    // EventSource not available or network blocked — fail silently
  }
}

function stopThoughtStream() {
  if (_sseSource) { try { _sseSource.close(); } catch(e){} _sseSource = null; }
  // Fade and clear after a short delay so user can see "reply ready"
