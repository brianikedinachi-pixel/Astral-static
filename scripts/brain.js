...
async function _speakConvo(text) {
  ...
  const _finish = () => {
    ...
    if (_convoMode && !_convoMuted && !_convoThinking) {
      ...
      // Always-on mic is already running continuously — no restart needed.
      // Just reset the status text and update the watchdog timestamp.
...
function _buildAlwaysOnRec() {
  ...
  r.onend = () => {
    ...
    // and re-acquiring while TTS is mid-playback can audibly crackle it.
    // A short settle gap fixes both without meaningfully hurting barge-in.
