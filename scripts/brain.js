...
/* ══════════════════════════════════════════════════════
   THOUGHT LOG — Claude-app style pill
   Single collapsed line with spinner. Tap to expand.
   Disappears when reply arrives.
══════════════════════════════════════════════════════ */
var _sseSource       = null;
var _logMessages     = [];
var _logExpanded     = false;
var _logDismissTimer = null;

function _stagePhrase(stage, msg) {
  if (stage === 'web') {
    var m = msg.match(/for:\s*(.+)/i);
    var q = m ? m[1].slice(0, 38) : msg.slice(0, 38);
    return 'Searching for \u201c' + q + (q.length >= 38 ? '\u2026' : '') + '\u201d';
  }
  if (stage === 'mem')    return 'Reading your history\u2026';
  if (stage === 'gemini') return 'Thinking\u2026';
  if (stage === 'req')    return 'Preparing\u2026';
  if (stage === 'reply')  return 'Almost ready\u2026';
  if (stage === 'error')  return 'Something went wrong\u2026';
  return 'Working\u2026';
}

function _stageFullLabel(stage, msg) {
  if (stage === 'web') {
    var m = msg.match(/for:\s*(.+)/i);
    return '\uD83D\uDD0D Searching for \u201c' + (m ? m[1].slice(0,50) : msg.slice(0,50)) + '\u201d';
  }
  if (stage === 'mem')    return '\uD83E\uDDE0 Reading your history';
  if (stage === 'gemini') return '\u2728 Thinking';
  if (stage === 'req')    return '\uD83D\uDCE8 Preparing request';
  if (stage === 'reply')  return '\u2705 Reply ready';
  if (stage === 'error')  return '\u26A0\uFE0F ' + msg.slice(0, 60);
  return '\u2699\uFE0F ' + msg.slice(0, 60);
}

function ensureLogOverlay() {
  if (document.getElementById('astral-thinking-pill')) return;

  var style = document.createElement('style');
  style.textContent =
    '#astral-thinking-pill{' +
      'position:fixed;bottom:82px;left:50%;transform:translateX(-50%);' +
      'z-index:8000;pointer-events:all;' +
      'display:none;' +
      'flex-direction:column;align-items:flex-start;' +
      'width:min(480px,88vw);' +
      'animation:pillSlideUp .2s ease forwards;' +
    '}' +
    '@keyframes pillSlideUp{from{opacity:0;transform:translateX(-50%) translateY(8px)}to{opacity:1;transform:translateX(-50%) translateY(0)}}' +
    '#astral-pill-row{' +
    ...
    '}' +
    '#astral-pill-row:active{background:rgba(0,212,255,0.08);}' +
    '#astral-pill-spinner{' +
    ...
    '}' +
    '#astral-pill-chevron{' +
      'font-size:0.65rem;color:rgba(0,212,255,0.35);flex-shrink:0;' +
      'transition:transform .2s;' +
    '}' +
    '#astral-thinking-pill.expanded #astral-pill-chevron{transform:rotate(180deg);}' +
    '#astral-pill-steps{' +
      'display:none;' +
    ...
    '}' +
    '#astral-thinking-pill.expanded #astral-pill-steps{display:block;}' +
    '.apill-step{' +
      'font-family:"DM Sans","Segoe UI",sans-serif;' +
      'font-size:0.74rem;color:rgba(180,210,255,0.42);' +
      'padding:3px 0;line-height:1.5;' +
      'white-space:nowrap;overflow:hidden;text-overflow:ellipsis;' +
    '}' +
    '.apill-step.latest{color:rgba(0,212,255,0.78);font-weight:500;}' +
    '@media(max-width:600px){#astral-thinking-pill{bottom:74px;}}';

  document.head.appendChild(style);

  var pill = document.createElement('div');
  pill.id = 'astral-thinking-pill';
  pill.innerHTML =
    '<div id="astral-pill-row">' +
      '<div id="astral-pill-spinner"></div>' +
      '<span id="astral-pill-text">Preparing\u2026</span>' +
      '<span id="astral-pill-chevron">\u25BE</span>' +
    '</div>' +
    '<div id="astral-pill-steps"></div>';
  document.body.appendChild(pill);

  document.getElementById('astral-pill-row').addEventListener('click', function() {
    pill.classList.toggle('expanded');
    _logExpanded = pill.classList.contains('expanded');
  });
}

function _updatePill(stage, msg) {
  var textEl  = document.getElementById('astral-pill-text');
  var stepsEl = document.getElementById('astral-pill-steps');
  if (!textEl || !stepsEl) return;
  textEl.textContent = _stagePhrase(stage, msg);
  var prev = stepsEl.querySelector('.apill-step.latest');
  if (prev) prev.classList.remove('latest');
  var step = document.createElement('div');
  step.className = 'apill-step latest';
  step.textContent = _stageFullLabel(stage, msg);
  stepsEl.appendChild(step);
  while (stepsEl.children.length > 8) stepsEl.removeChild(stepsEl.firstChild);
}

function _showPill() {
  var el = document.getElementById('astral-thinking-pill');
  if (el) { el.style.display = 'flex'; el.classList.remove('expanded'); }
}
...
function _hidePill() {
  ...
  setTimeout(function() {
    ...
    el.style.opacity = '';
    el.style.transition = '';
    var steps = document.getElementById('astral-pill-steps');
    if (steps) steps.innerHTML = '';
  }, 380);
}

function _pushLog(entry) {
  _logMessages.push(entry);
  if (_logMessages.length > 20) _logMessages.shift();
  _updatePill(entry.stage, entry.msg || '');
}

function _clearLog() { _logMessages = []; }
...
function startThoughtStream() {
  ...
  _showPill();
  try {
    _sseSource = new EventSource(SERVER_URL + '/stream-log');
    _sseSource.onmessage = function(e) {
      try { _pushLog(JSON.parse(e.data)); } catch(err) {}
    };
    _sseSource.onerror = function() {
      if (_sseSource) { try { _sseSource.close(); } catch(ex){} _sseSource = null; }
    };
  } catch(e) {}
}
...
function stopThoughtStream() {
  if (_sseSource) { try { _sseSource.close(); } catch(e){} _sseSource = null; }
  _logDismissTimer = setTimeout(function() {
    _hidePill();
    _clearLog();
  }, 900);
}
/* ══════════════════════════════════════════════════════
   SEND MESSAGE  (with thought-log integration)
══════════════════════════════════════════════════════ */
async function handleSend() {
  ...
  // ── Start live thought stream ──
  startThoughtStream();

  try {
    // Build last 20 messages as conversation history for full context
    var convHistory = [];
    ...
    // ── Stop thought stream (reply arrived) ──
    stopThoughtStream();
...
/* ══════════════════════════════════════════════════════
   GLOBAL EXPORTS — used by chat.html inline scripts
══════════════════════════════════════════════════════ */
