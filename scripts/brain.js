...
function _prefetchConvoTTS(text) {
  ...
  _convoTTSChunks    = chunks.map(t => ({ text: t, promise: _fetchTTSChunk(t) }));
...
async function _speakConvo(text) {
  ...
  // Use the pipelined chunks if they were prefetched for this exact text;
  // otherwise (e.g. an error message that skipped _prefetchConvoTTS) fetch
  // one chunk now so it still gets spoken instead of playing silently.
  let chunks = (_convoTTSChunksFor === text && _convoTTSChunks.length)
    ? _convoTTSChunks
    : [{ text, promise: _fetchTTSChunk(_prepareTextForSpeech(text)) }];
  _convoTTSChunks    = [];
  _convoTTSChunksFor = '';
  _convoTTSPrefetch  = null;
  ...
  for (let ci = 0; ci < chunks.length; ci++) {
    ...
    try { blob = await chunks[ci].promise; } catch(e) {}
    if (pulse) clearInterval(pulse);
    if (sig.aborted || !_convoMode) break;
    if (!blob) continue; // this sentence's synthesis failed — keep going with the rest

    playedAny = true;
    const chunkWordCount = chunks[ci].text.replace(/\s+/g, ' ').trim().split(' ').length;
    const baseIdx    = wordIdx;
    const targetIdx  = Math.min(words.length, baseIdx + chunkWordCount);
    ...
    await new Promise((resolve) => {
      ...
      audio.addEventListener('timeupdate', () => {
        ...
        const target = baseIdx + Math.floor((audio.currentTime / audio.duration) * chunkWordCount);
    ...
    wordIdx = targetIdx; // make sure we've reached this chunk's boundary even if the ticker lagged
  }
...
async function startConvoMode() {
  ...
  if (!_isPWA && !_isVIP) {
    ...
    if (chip) {
      ...
      chip.style.boxShadow = '0 0 0 3px rgba(0,234,255,0.5)';
      setTimeout(() => { chip.style.boxShadow = ''; }, 2000);
    }
    return;
  }
