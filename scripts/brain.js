...
/* ══════════════════════════════════════════════════════
   ...
   NEW: Emotion detection adapts Gemini tone per turn.
...
let _convoSpeaking    = false; // true while AI audio (filler or real reply) is actually playing — drives barge-in
let _convoMicFailCount = 0;    // consecutive mic start/recognition failures — drives retry backoff
...
function _stopConvoAudio() { _masterTTSStop(); _stopFillerAudio(); _convoSpeaking = false; _convoTTSPrefetch = null; }

// Fire TTS fetch immediately when reply text is known — before anything else runs
...
// Curated, TTS-friendly "thinking" fillers — natural human hesitation markers
// and short bridge phrases (the "um/uh/hmm/let me think" family linguists call
// filled pauses + conversation fillers), trimmed to avoid pure onomatopoeia
// ("Mmm", "Mm-hmm") that can mangle badly on the Google-Translate TTS fallback.
const _CONVO_THINKING_SOUNDS = [
  ...
  "Okay, let me see.",
  "Hang on, thinking.",
  "Let me weigh that for a second.",
  "Sure thing, one moment.",
];

/* ── Persistent "preset" filler audio ─────────────────────────────────────────
   Filler phrases never change, so their audio is fetched from the Gemini TTS
   proxy at most ONCE per device — ever — then stored permanently in the Cache
   Storage API (survives reloads, app restarts, even offline). Every convo mode
   activation after the first gets these instantly with zero network round trip,
   which is what makes them feel "preset" rather than freshly generated. */
const _FILLER_CACHE_NAME = 'astral-filler-tts-v1';
const _fillerAudioCache = new Map(); // phrase → Promise<Blob|null>, in-memory for this page life
...
async function _getFillerAudioBlob(phrase) {
  ...
  const promise = (async () => {
    // 1) Already generated in a previous session? Instant, no network needed.
    try {
      if ('caches' in window) {
    ...
    // 2) First time ever for this phrase — fetch once, then persist it.
    try {
      const resp = await fetch(_SERVER_URL_TTS, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      } catch(e) {} // persistence is best-effort; in-memory cache still works this session
...
function _prefetchFillerAudio() {
  ...
  // Fire all lookups in parallel — non-blocking, best-effort. Phrases already
  // persisted from a past session resolve almost immediately from Cache Storage;
  // only genuinely new phrases hit the network.
  for (const phrase of _CONVO_THINKING_SOUNDS) {
    _getFillerAudioBlob(phrase);
  }
}
...
async function _convoThinkFiller() {
  ...
  // Avoid playing the same filler twice in a row
  if (keys.length > 1) {
    let guard = 0;
  ...
  try {
    const blob = await _getFillerAudioBlob(phrase); // instant if persisted, else awaits one-time fetch
    if (!blob || !_convoThinking) return; // not ready, or the real reply already arrived
    _stopFillerAudio(); // safety: never overlap a previous filler
    const url = URL.createObjectURL(blob);
    const audio = new Audio(url);
    window._fillerAudioEl = audio; // tracked so the real reply's TTS can cut it off cleanly
...
function _setConvoState(state) {
  ...
  if (!wrap) return;
  wrap.classList.remove('state-listening', 'state-thinking', 'state-speaking', 'state-analyzing');
  ...
  if (status) {
    status.textContent =
      ...
      state === 'analyzing' ? 'Reading your mood…' :
      'Listening…';
  }
}
...
function _triggerWordPulse() {
  ...
  // Small background orb (shown when the user has stepped away) glows too
  const mini = document.getElementById('convo-orb-mini');
  if (mini) {
    mini.classList.remove('word-pulse');
    void mini.offsetWidth;
    mini.classList.add('word-pulse');
  }
}

/* ── Main convo TTS — plays pre-fetched Gemini audio instantly ──────────────── */
// TTS fetch is kicked off the moment AI reply text arrives (_prefetchConvoTTS),
// so by the time this runs the blob is ready (or milliseconds away).
// Word-by-word subtitles are driven by audio.currentTime progress — zero Web Speech.
async function _speakConvo(text, isFiller) {
  if (isFiller) return; // orb animation shows thinking — no audio filler
  ...
  // Await pre-fetched blob (started when reply arrived — usually ready instantly)
  let blob = null;
  ...
  const _finish = () => {
    ...
    if (_convoMode && !_convoMuted) {
      ...
      setTimeout(_convoStartListening, 250);
    }
  };

  if (!blob) { _finish(); return; } // server down — show text, no audio

  // ── Play Gemini blob; sync word-by-word subtitles to audio.currentTime ──
  _masterAbortCtrl = new AbortController();
  const sig = _masterAbortCtrl.signal;
  const url = URL.createObjectURL(blob);
  const audio = new Audio(url);
  ...
  await new Promise((resolve) => {
    ...
    // timeupdate fires ~4x/sec — enough resolution for word-level subtitles
...
function _convoAddBubble(role, text) {
  ...
  if (role === 'user') {
    ...
    // Push empty AI bubble — words will stream into it via _speakConvo
  ...
  saveToStorage(); // persist immediately — don't wait on the AI reply to land
  renderMessages();
  scrollToBottom();
}

/* ── Send to AI with emotion ────────────────────────────────────────────────── */
async function _convoSendToAI(userText) {
  // Barge-in: interrupt AI if it's mid-speech
  if (_convoSpeaking) {
    _stopConvoAudio();
    speechSynthesis.cancel();
  }
  ...
  // Show user bubble immediately — _convoAddBubble persists it to storage right away
  _convoAddBubble('user', userText);

  _convoThinkFiller();
  _setConvoState('thinking');

  // ── Emotion detection runs in parallel while we prep the request ──────────
  ...
  try {
    ...
    const body = {
      ...
      conversation_history: _convoHistory.slice(-8),
      // Emotion forwarded to server for Gemini tone adaptation
    ...
    _convoThinking = false;
    _prefetchConvoTTS(reply); // ← TTS fetch starts NOW, before bubble renders
    // Push empty AI bubble; _speakConvo streams words into it
    _convoAddBubble('ai', reply);
    speechSynthesis.cancel();
    _speakConvo(reply, false);

    // Restart emotion capture ready for next user turn
    if (_convoMode) setTimeout(_startEmotionCapture, 200);

  } catch(e) {
    _convoThinking = false;
    const errText = e.name === 'AbortError'
      ? "Sorry, that took too long. Could you say it again?"
      : "Sorry, something went wrong on my end. Say that again?";
    _convoAddBubble('ai', errText);
    _speakConvo(errText, false);
  }
}

/* ── Listening ──────────────────────────────────────────────────────────────── */
let _convoListenTimer = null;
const _CONVO_MIC_MAX_FAILS = 6;

// Backoff grows with each consecutive failure (300ms → 600 → 1200 → 2400 → 4800 → 8000 cap).
// Without this, a persistent mic error (hardware busy, transient OS-level denial, etc.)
// would retry every ~300ms forever — r.onend always re-fires right after r.onerror and
// used to blindly reschedule at a flat 300ms, masking any backoff and spamming retries.
function _convoMicRetryDelay() {
  return Math.min(300 * Math.pow(2, _convoMicFailCount), 8000);
}

function _convoGiveUpListening() {
  _convoMicFailCount = 0;
  _convoMuted = true; // pause instead of fully ending — preserves the conversation/history
  document.getElementById('convo-orb-wrap')?.classList.add('state-muted');
  const status = document.getElementById('convo-status-text');
  if (status) status.textContent = 'Lost the mic — tap to try again.';
}
...
function _convoStartListening() {
  ...
  if (_convoRecognition) {
    try { _convoRecognition.onend = null; _convoRecognition.abort(); } catch(e) {}
    _convoRecognition = null;
  }

  _setConvoState('listening');
  _startEmotionCapture(); // capture audio while user speaks
  ...
  r.onstart = () => { _convoMicFailCount = 0; }; // mic is demonstrably working again
    ...
    for (let i = ev.resultIndex; i < ev.results.length; i++) {
      if (ev.results[i].isFinal) {
        ...
        for (let j = 0; j < ev.results[i].length; j++) {
          if (ev.results[i][j].confidence > bestC) {
            bestC = ev.results[i][j].confidence;
        ...
        interim += ev.results[i][0].transcript;
        const status = document.getElementById('convo-status-text');
        if (status && !_convoThinking) status.textContent = interim ? `"${interim}…"` : 'Listening…';
      }
    }
    if (final_.trim()) {
      const status = document.getElementById('convo-status-text');
  ...
  r.onerror = (ev) => {
    ...
    if (_convoMode && !_convoMuted && !_convoThinking) _scheduleConvoRestart(_convoMicRetryDelay());
  };

  r.onend = () => {
    _convoRecognition = null;
    if (_convoMode && !_convoThinking && !_convoMuted) {
      // Respect backoff if the preceding attempt failed; otherwise this is just the
      // normal end-of-utterance restart for continuous listening.
      _scheduleConvoRestart(_convoMicFailCount > 0 ? _convoMicRetryDelay() : 300);
    }
  };
  ...
  try { r.start(); } catch(e) {
    ...
    _convoMicFailCount++;
    if (_convoMicFailCount >= _CONVO_MIC_MAX_FAILS) { _convoGiveUpListening(); return; }
    if (_convoMode && !_convoThinking && !_convoMuted) _scheduleConvoRestart(_convoMicRetryDelay());
  }
}
...
/* ── Are we running as the installed app, or just a browser tab? ───────────
   The PWA manifest uses display:standalone, so once installed (Android/
   desktop "Install App", or iOS "Add to Home Screen") the page runs in
   standalone/fullscreen mode. A plain browser tab on the website does not.
   Conversation mode needs continuous mic + background behaviour that only
   really works once installed, so we gate it here. */
function _isInstalledApp() {
  ...
  if (document.referrer && document.referrer.startsWith('android-app://')) return true; // TWA
  return false;
}

/* ── Start / Stop ───────────────────────────────────────────────────────────── */
async function startConvoMode() {
  // Dev/owner bypass — allow convo mode in browser for this account
  const _session = (() => { try { return JSON.parse(localStorage.getItem('astral_session') || 'null'); } catch(e) { return null; } })();
  const _isBypassUser = _session?.email === 'bukanwoko@gmail.com';

  if (!_isBypassUser && !_isInstalledApp()) {
    _showGetAppPopup();
    return;
  }

  const SR = window.SpeechRecognition || window.webkitSpeechRecognition;
  if (!SR) { alert('Voice conversation requires Chrome, Edge, or Safari.'); return; }
  ...
  try {
    ...
    alert('Microphone access is required for conversation mode. Please allow mic access and try again.');
    return;
  }
  ...
  _convoSpeaking     = false;
  _convoMicFailCount = 0;
  _activeConvoBubbleIdx = -1; // guard against a stale index left over from a previous session
  _spokenWords       = [];
  _lastEmotionResult = { emotion: 'neutral', confidence: 0.5 };
  _backendTTSAvailable = null;

  // Kick off all filler TTS fetches immediately in parallel — non-blocking
  ...
  await _registerConvoNotification();

  setTimeout(() => {
    const greeting = "Hey, I'm listening. What's on your mind?";
    _convoAddBubble('ai', greeting); // so the greeting actually shows up in history + gets saved
    _speakConvo(greeting, false);
  }, 300);
}
...
function _injectConvoOrb() {
  ...
  wrap.innerHTML = `
    ...
    <div class="convo-orb-stack">
      ...
      <button id="convo-orb-btn" class="convo-orb-btn" type="button" aria-label="Conversation active — tap to stop">
...
function _showGetAppPopup() {
  ...
  popup.innerHTML = `
    ...
    <div class="gap-box">
      ...
      <p class="gap-sub">Talking with Astral — real voice, always listening, even with your screen off — only works in the Astral app. Install it to start your conversation.</p>
...
function _installOrClose() {
  if (typeof installApp === 'function') installApp();
...
/* ── Background-notification heartbeat ────────────────────────────────────────
   A service worker's own internal setInterval cannot be trusted to keep firing:
   browsers are free to terminate an idle SW at any time (often within seconds),
   silently killing that timer along with it — this was the root cause of
   notifications going quiet during a backgrounded conversation. Instead, the
   page itself (which stays alive longer while merely backgrounded, not closed)
   pings the SW on a timer; each ping re-activates it and triggers a fresh
   "is a notification due" check. */
