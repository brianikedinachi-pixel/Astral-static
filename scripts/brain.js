...
// Stop any in-progress TTS (backend audio or browser fallback)
...
// ── Browser Web Speech — best-voice selection ────────────────────────────────
// Uses the same _pickBestVoice() already defined above in the VOICE section.
// Natural settings tuned for conversation (slightly slower, warmer pitch).
function _speakBrowserTTS(text, onDone) {
  if (!text) { if (onDone) onDone(); return; }
  speechSynthesis.cancel();
  const u = new SpeechSynthesisUtterance(text);
  u.rate   = 0.95;   // slightly slower than normal — more natural pacing
  u.pitch  = 0.95;   // slightly lower — warmer, less robotic
  u.volume = 1;
  ...
  // Pulse orb on word boundaries for visual feedback
...
// ── Backend TTS proxy (ElevenLabs when ELEVENLABS_API_KEY is set server-side)
// Falls back seamlessly to browser TTS if backend returns error / no key set.
async function _speakBackendTTS(text, onDone) {
  try {
    ...
    if (!resp.ok) throw new Error('no tts');
    const blob = await resp.blob();
    ...
    audio.onerror = () => {
      _currentAudio = null;
      URL.revokeObjectURL(url);
      _speakBrowserTTS(text, onDone);
    };
    audio.play();
  } catch(e) {
    // Backend TTS not available — use browser
    _speakBrowserTTS(text, onDone);
  }
}

// Main convo speak — tries backend TTS first, falls back to browser
function _speakConvoVoice(text, onDone) {
  // Try backend TTS (ElevenLabs if configured, else falls back automatically)
  _speakBackendTTS(text, onDone);
}
