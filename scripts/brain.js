...
async function startConvoMode() {
  ...
  // Conversation mode is currently restricted to one account while it's
  // being stabilised. Everyone else is pointed at the installed app instead
  // of getting a half-working in-browser session.
  const _CONVO_ALLOWED_EMAIL = 'bukanwoko@gmail.com';
  let _convoSessionObj = null;
  try { _convoSessionObj = JSON.parse(localStorage.getItem('astral_session') || 'null'); } catch(e) {}
  const _convoUserEmail = (_convoSessionObj?.email || '').trim().toLowerCase();
  if (_convoUserEmail !== _CONVO_ALLOWED_EMAIL) {
    _showConvoError('Conversation mode needs the Astral app — tap the menu → "Get App" to install it.');
    return;
  }

  // Conversation mode now runs the same way on web as it does in the
  // installed app — no install gate. Anyone who wants the app (background
  // listening, notifications, etc.) can still get it from the "Get App"
  // link in the menu, which points at get-app.html.
