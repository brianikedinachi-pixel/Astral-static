...
// ── ElevenLabs via Puter.js (free, no API key) ───────────────────────────────
...
function _loadPuter(cb) {
  if (_puterLoaded) { cb(); return; }
  _puterCallbacks.push(cb);
  ...
  s.onload = () => {
    _puterLoaded = true;
    _puterLoading = false;
    _puterCallbacks.forEach(fn => fn());
    _puterCallbacks = [];
  };
  s.onerror = () => {
    _puterLoading = false;
    _puterCallbacks.forEach(fn => fn(true)); // pass error flag
    _puterCallbacks = [];
  };
  document.head.appendChild(s);
}

// Voice IDs — ElevenLabs public voices that sound natural
// Rachel (21m00Tcm4TlvDq8ikWAM) — warm female, great for conversation
// Adam  (pNInz6obpgDQGcFmaJgB) — natural male
const _EL_VOICE_ID    = '21m00Tcm4TlvDq8ikWAM'; // Rachel — warm, human-sounding
const _EL_MODEL       = 'eleven_flash_v2_5';      // fastest + still high quality

// Stop any in-progress TTS (both ElevenLabs and browser fallback)
...
// Speak using ElevenLabs TTS via puter.js; falls back to browser TTS on error
function _speakElevenLabs(text, onDone) {
  _loadPuter((err) => {
    if (err || typeof puter === 'undefined') {
      _speakBrowserFallback(text, onDone);
      return;
    }
    try {
      puter.ai.txt2speech(text, {
        ...
        if (!_convoMode) { if (onDone) onDone(); return; }
        _currentAudio = audio;
        audio.onended = () => { _currentAudio = null; if (onDone) onDone(); };
        audio.onerror = () => { _currentAudio = null; _speakBrowserFallback(text, onDone); };
        audio.play();
      }).catch(() => {
        _speakBrowserFallback(text, onDone);
      });
...
// Browser speech synthesis fallback with best available voice
function _speakBrowserFallback(text, onDone) {
  const u = new SpeechSynthesisUtterance(text);
  u.rate = 0.97; u.pitch = 0.95; u.volume = 1;
  const v = _pickBestVoice ? _pickBestVoice() : null;
  if (v) u.voice = v;
  ...
  if (!speechSynthesis.getVoices().length) {
    speechSynthesis.addEventListener('voiceschanged', function once() {
      speechSynthesis.removeEventListener('voiceschanged', once);
      const v2 = _pickBestVoice ? _pickBestVoice() : null;
      if (v2) u.voice = v2;
      speechSynthesis.speak(u);
    }, { once: true });
  } else {
...
function _convoThinkFiller() {
  ...
  // Use browser TTS for fillers (fast, no latency) — ElevenLabs for real replies
  _speakBrowserFallback(r, null);
}
...
// Main convo TTS — uses ElevenLabs for real replies, browser TTS for fillers
function _speakConvo(text, isFiller) {
  if (isFiller) {
    _speakBrowserFallback(text, null);
    return;
  }

  _setConvoState('speaking');
  _stopConvoAudio(); // stop any filler or prior speech

  // Start fallback pulse animation while waiting for ElevenLabs
  let fallbackPulse = setInterval(_triggerWordPulse, 260);
  const stopPulse = () => { clearInterval(fallbackPulse); fallbackPulse = null; };

  // Start listening in background so user can barge in while AI speaks
  setTimeout(() => {
