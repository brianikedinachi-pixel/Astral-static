...
/* ══════════════════════════════════════════════════════
   TOP USER HIGHLIGHT
   Shows a small banner to the #1 user by message count
══════════════════════════════════════════════════════ */
async function checkTopUserStatus() {
  if (!session || !session.email) return;
  try {
    const r = await fetch(SERVER_URL + '/admin-stats?admin_email=check_top&user_email=' + encodeURIComponent(session.email));
    if (!r.ok) return;
    const d = await r.json();
    if (d.is_top_user) {
      showTopUserBanner(d.top_message_count || 0);
    }
  } catch(e) {}
}

function showTopUserBanner(count) {
  if (document.getElementById('top-user-banner')) return; // already shown
  const banner = document.createElement('div');
  banner.id = 'top-user-banner';
  banner.innerHTML =
    '<span style="font-size:1.1rem;">🏆</span>' +
    '<span>You\'re Astral\'s #1 user with <strong>' + count + ' messages</strong>! Thank you for being here — you inspire this whole journey. 💙</span>' +
    '<button onclick="this.parentElement.remove()" style="background:none;border:none;color:inherit;cursor:pointer;font-size:1rem;opacity:0.7;margin-left:auto;padding:0 4px;">✕</button>';
  Object.assign(banner.style, {
    display: 'flex',
    alignItems: 'center',
    gap: '10px',
    padding: '12px 16px',
    background: 'linear-gradient(90deg, rgba(251,191,36,0.13), rgba(245,158,11,0.08))',
  ...
  // Insert before the chat display
  var container = displayContainer || document.querySelector('main') || document.body;
  container.parentNode && container.parentNode.insertBefore(banner, container);
}
...
function ensureCommentModal() {
  ...
  modal.innerHTML =
    '<div id="astral-comment-overlay" style="position:fixed;inset:0;background:rgba(0,0,0,0.6);backdrop-filter:blur(4px);z-index:9998;"></div>' +
    '<div id="astral-comment-box" style="position:fixed;left:50%;top:50%;transform:translate(-50%,-50%);z-index:9999;background:#0c1120;border:1px solid rgba(0,212,255,0.2);border-radius:18px;padding:1.6rem 1.5rem;width:min(480px,92vw);box-shadow:0 24px 60px rgba(0,0,0,0.6);">' +
      '<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:1rem;">' +
        '<h3 style="font-size:0.95rem;font-weight:700;color:#e2eeff;margin:0;">💬 Leave a comment</h3>' +
        '<button id="astral-comment-close" style="background:none;border:none;color:#94a3b8;cursor:pointer;font-size:1.2rem;padding:4px;">✕</button>' +
      '</div>' +
      '<div id="astral-comment-prev-wrap" style="margin-bottom:1rem;"></div>' +
      '<textarea id="astral-comment-input" placeholder="Write your comment here…" style="width:100%;min-height:88px;padding:0.85rem 1rem;background:rgba(0,212,255,0.04);border:1px solid rgba(0,212,255,0.15);border-radius:12px;color:#e2eeff;font-family:inherit;font-size:0.875rem;resize:vertical;outline:none;line-height:1.5;box-sizing:border-box;"></textarea>' +
      '<div style="display:flex;gap:10px;margin-top:0.9rem;justify-content:flex-end;">' +
        '<button id="astral-comment-cancel" style="padding:0.6rem 1.2rem;border-radius:10px;background:none;border:1px solid rgba(0,212,255,0.15);color:#94a3b8;cursor:pointer;font-family:inherit;font-size:0.85rem;">Cancel</button>' +
      ...
      '</div>' +
      '<div id="astral-comment-status" style="display:none;margin-top:0.7rem;font-size:0.8rem;color:#34d399;"></div>' +
    '</div>';
  document.body.appendChild(modal);
  document.getElementById('astral-comment-overlay').addEventListener('click', closeCommentModal);
  document.getElementById('astral-comment-close').addEventListener('click', closeCommentModal);
  document.getElementById('astral-comment-cancel').addEventListener('click', closeCommentModal);
}

let _commentTargetIdx = null;

function openCommentModal(idx) {
  ensureCommentModal();
  _commentTargetIdx = idx;
  ...
  if (existing.length > 0) {
    prevWrap.innerHTML = '<div style="font-size:0.72rem;color:#475569;text-transform:uppercase;letter-spacing:0.08em;font-weight:600;margin-bottom:8px;">' + existing.length + ' comment' + (existing.length>1?'s':'') + ' so far</div>' +
  ...
  setTimeout(function() { var el = document.getElementById('astral-comment-input'); if(el) el.focus(); }, 100);

  // Wire submit
  var submitBtn = document.getElementById('astral-comment-submit');
  submitBtn.onclick = function() { submitComment(idx); };
}

function closeCommentModal() {
  var modal = document.getElementById('astral-comment-modal');
  if (modal) modal.style.display = 'none';
  _commentTargetIdx = null;
}
...
async function submitComment(idx) {
  ...
  try {
    ...
    if (data && data.ok) {
      // Store comment locally in the message object
      if (!log.comments) log.comments = [];
      log.comments.push(data.comment);
      saveToStorage();
      renderMessages();
      if (status) { status.textContent = '✅ Comment posted!'; status.style.display = 'block'; }
      if (input) input.value = '';
      setTimeout(closeCommentModal, 1200);
    } else {
      if (status) { status.textContent = '❌ Failed to post. Try again.'; status.style.color = '#fb7185'; status.style.display = 'block'; }
    }
  } catch(e) {
    // Save locally even if backend fails
    if (!log.comments) log.comments = [];
    log.comments.push({ id: Date.now().toString(), ts: new Date().toISOString(), user_email: (session&&session.email)||'', user_name: (session&&session.name)||'', text: text });
    saveToStorage();
    renderMessages();
    if (status) { status.textContent = '💾 Saved locally (offline).'; status.style.display = 'block'; }
    if (input) input.value = '';
...
async function init() {
  ...
  if (!loaded || chatHistory.length === 0) {
    ...
    loadChatsUI();
    renderMessages();
    // Load comments from backend for current chat messages
    loadCommentsFromBackend();
  }

  // Check if this user is the top user (non-blocking)
  checkTopUserStatus();
}
