...
async function startConvoMode() {
  ...
  // Conversation mode runs on web for everyone. The installed app is still
  // the better experience for it (background listening survives the screen
  // turning off / the tab losing focus, which a browser tab can't fully
  // guarantee), so nudge web users toward installing — but don't block them.
  const _convoIsInstalled = () =>
  ...
  if (!_convoIsInstalled()) {
    try {
      if (!sessionStorage.getItem('astral_convo_web_nudge_shown')) {
        sessionStorage.setItem('astral_convo_web_nudge_shown', '1');
        showToast('💡 Convo Mode works best in the installed app — tap ⋮ → Get App');
      }
    } catch (e) {}
  }
