...
import { startBargeInWatch, stopBargeInWatch, endOfTurnDelayFor,
         startBackchannelPulses, stopBackchannelPulses } from './convo/convo-turntaking.js';
import { extractSignature, matchOrRegisterSpeaker, needsIntroduction,
         introductionPrompt, markIntroduced, getRoster, resetRoster } from './convo/convo-speakers.js';

const SERVER_URL = 'https://astral-1-sb1i.onrender.com';

// Fire a lightweight, fire-and-forget ping the instant this script loads.
// Render free-tier spins the backend down after ~15min idle, and waking it
// back up can take 30-50s+ on its own — this gives that wake-up a head
// start before the person has even finished reading the screen, typing,
// or opening their mouth, instead of eating into the /chat or /convo-chat
// request's own timeout budget. Errors are ignored — this is purely
// best-effort and never blocks anything else from loading.
fetch(SERVER_URL + '/health', { method: 'GET' }).catch(() => {});
...
function loadSession() {
  try {
    ...
    // Background check — confirms the 7-day session is still valid and the
    // account hasn't since been banned. Non-blocking: local session is
    // trusted instantly for a fast load, this just double-checks quietly.
    if (session.token) {
      fetch(SERVER_URL + '/session-check?token=' + encodeURIComponent(session.token))
...
window.doLogout = () => {
  try {
    const s = JSON.parse(localStorage.getItem('astral_session') || 'null');
    if (s && s.token) {
      // Best-effort — invalidate the token server-side so it can't be
      // replayed elsewhere. Don't block logout if this fails.
...
function _startTypewriter() {
  ...
  if (!el) return;
  if (_twRunning) return; // already cycling — don't reset mid-phrase/mid-pause
  _twRunning = true;
  if (_twTimer) { clearTimeout(_twTimer); _twTimer = null; }
  ...
  const phrases = [
    ...
    { text: 'support that never clocks out',   pause: 2000 },
...
async function _speakConvo(text) {
  ...
  // Mic is fully off for full recognition while the AI talks — but a
  // lightweight energy-only watch stays on so the user can cut in, the way
  // people actually interrupt each other (see convo/convo-turntaking.js).
  // It snaps back on the instant playback finishes (see _finish() below).
  _convoStopListening();
  startBargeInWatch(() => {
    // User cut in mid-reply — stop the AI immediately and hand off to full
    // listening, same cleanup _commitAon()'s existing barge-in path uses.
    _stopConvoAudio();
    speechSynthesis.cancel();
    _masterAbortCtrl?.abort();
  ...
  const _finish = () => {
    ...
    stopBargeInWatch(); // one-shot per turn — the AI finished on its own, no interrupt happened
    _convoThinking = false;
    _convoSpeaking = false;
...
async function _convoSendToAI(userText) {
  ...
  try {
    ...
    const body = {
      ...
      // Approximate voice-cluster label, only meaningful once >1 speaker has
      // been detected this session — see convo/convo-speakers.js.
      speaker_label:        getRoster().length > 1 ? (_convoActiveSpeaker?.label || null) : null,
    };
...
/* ── Lightweight rolling audio buffer for speaker-signature extraction ────────
   SpeechRecognition never exposes raw samples, so multi-party tracking
   (convo/convo-speakers.js) needs its own tiny mic tap purely to read
   pitch/timbre. This is a SEPARATE stream from _alwaysOnRec's — two mic
   consumers at once is normally fine, but on flaky Android OEM browsers
   (see the watchdog comments below) it's one more thing that can wobble.
   Every failure here is caught and swallowed: if it can't open, speaker
   tracking just silently stays off and everything else in convo mode is
   unaffected. */
let _speakerAudioCtx  = null;
let _speakerProcessor = null;
let _speakerStream    = null;
let _speakerRing      = null;
let _speakerRingLen   = 0;
let _speakerRingPos   = 0;
let _convoActiveSpeaker = null; // current best-guess speaker for the in-flight utterance

async function _startSpeakerTap() {
  if (_speakerAudioCtx) return;
  try {
    _speakerStream = await navigator.mediaDevices.getUserMedia({ audio: true });
  } catch(e) { return; }
  try {
    _speakerAudioCtx = new (window.AudioContext || window.webkitAudioContext)();
    const source = _speakerAudioCtx.createMediaStreamSource(_speakerStream);
    _speakerRingLen = Math.floor(_speakerAudioCtx.sampleRate * 1.2); // ~1.2s of tail audio per utterance
    _speakerRing    = new Float32Array(_speakerRingLen);
    _speakerRingPos = 0;
    // ScriptProcessorNode is deprecated but universally supported; it only
    // fires onaudioprocess once connected through to a destination, so we
    // route it through a silent gain node — never audible, never a feedback loop.
    _speakerProcessor = _speakerAudioCtx.createScriptProcessor(2048, 1, 1);
    _speakerProcessor.onaudioprocess = (ev) => {
      const chunk = ev.inputBuffer.getChannelData(0);
      for (let i = 0; i < chunk.length; i++) {
        _speakerRing[_speakerRingPos] = chunk[i];
        _speakerRingPos = (_speakerRingPos + 1) % _speakerRingLen;
      }
    };
    const silentGain = _speakerAudioCtx.createGain();
    silentGain.gain.value = 0;
    source.connect(_speakerProcessor);
    _speakerProcessor.connect(silentGain);
    silentGain.connect(_speakerAudioCtx.destination);
  } catch(e) { _stopSpeakerTap(); }
}

function _stopSpeakerTap() {
  if (_speakerProcessor) { try { _speakerProcessor.disconnect(); } catch(e) {} _speakerProcessor = null; }
  if (_speakerAudioCtx)  { try { _speakerAudioCtx.close(); } catch(e) {} _speakerAudioCtx = null; }
  if (_speakerStream)    { _speakerStream.getTracks().forEach(t => t.stop()); _speakerStream = null; }
  _speakerRing = null;
  _convoActiveSpeaker = null;
}

// Unwrap the ring buffer into chronological order for signature extraction.
function _snapshotSpeakerBuffer() {
  if (!_speakerRing) return null;
  const out = new Float32Array(_speakerRingLen);
  out.set(_speakerRing.subarray(_speakerRingPos));
  out.set(_speakerRing.subarray(0, _speakerRingPos), _speakerRingLen - _speakerRingPos);
  return out;
}
...
/* ── Fold un-finalized speech into the committed buffer before it's lost ──────
   Continuous recognition doesn't run as one unbroken session — engines
   (especially Chrome/Android) cycle it periodically even mid-utterance, and
   it can also drop from a network hiccup. Only `_aonPendingFinal` (already-
   finalized text) used to survive that; whatever the user was still saying —
   live in `_aonInterimLatest`, not yet finalized — was silently discarded
   the moment the old session ended and a new one began. This is what made
   words go missing mid-sentence. Call this right before any restart/teardown
   so the trailing, not-yet-finalized words get folded into the buffer
   instead of vanishing. */
function _foldInterimIntoPending() {
  const interim = _aonInterimLatest.trim();
  if (interim) {
    _aonPendingFinal  = _dedupAppend(_aonPendingFinal, interim) + ' ';
    _aonInterimLatest = '';
  }
}
...
function _commitAon() {
  ...
  // Multi-party: rough voice-signature match against the in-session roster
  // (see convo/convo-speakers.js — approximate clustering, not real ID).
  // Failure of any kind here just leaves _convoActiveSpeaker as-is; it's
  // never allowed to block sending the actual message.
  try {
    const buf = _snapshotSpeakerBuffer();
    if (buf && _speakerAudioCtx) {
      const sig = extractSignature(buf, _speakerAudioCtx.sampleRate);
      if (sig.pitch) {
        const { speaker, isNewSpeaker } = matchOrRegisterSpeaker(sig);
        _convoActiveSpeaker = speaker;
        if (isNewSpeaker && needsIntroduction(speaker)) {
          // Ask once per new voice, don't nag on every subsequent turn.
          markIntroduced(speaker.id);
          speak(introductionPrompt(), null, true);
        }
      }
    }
  } catch(e) {}
...
function _buildAlwaysOnRec() {
  ...
  r.onstart = () => {
    ...
    // Subtle "I'm still here" pulse on the orb during long user turns —
    // visual only, never audio, so it can never step on the user's turn.
    if (!_convoSpeaking) {
      startBackchannelPulses(() => {
        document.getElementById('convo-orb-wrap')?.classList.add('pulse-ack');
        setTimeout(() => document.getElementById('convo-orb-wrap')?.classList.remove('pulse-ack'), 260);
      });
    }
  };
  ...
  r.onresult = (ev) => {
    ...
    // Restart the "user is done talking" clock on ANY activity — interim or
    // final — not just finals. A long utterance often gets finalized into
    // several chunks by the recognizer while the user keeps talking through
    // it; if only finals reset this timer, the clock keeps running against
    // fresh interim speech and can fire mid-sentence. Gating on interim
    // events too means the countdown only ever completes once results have
    // genuinely stopped arriving — real silence, not mid-utterance chunking.
    if (_aonPendingFinal.trim()) {
      if (_aonSilenceTimer) clearTimeout(_aonSilenceTimer);
      _aonSilenceTimer = setTimeout(_commitAon, endOfTurnDelayFor(_aonPendingFinal));
    }
    _aonUpdateStatus();
  ...
  r.onend = () => {
    if (!_convoMode || _convoMuted) return;
    // Recognition can end mid-utterance (session cycling, brief network
    // hiccup) before trailing words the user was still saying ever got
    // finalized — recover them instead of losing them (see
    // _foldInterimIntoPending above).
    _foldInterimIntoPending();
...
function _convoStartListening() {
  ...
  if (_alwaysOnRec) {
    _foldInterimIntoPending();
    try { _alwaysOnRec.onstart = _alwaysOnRec.onresult = _alwaysOnRec.onsoundstart = _alwaysOnRec.onaudiostart = _alwaysOnRec.onend = _alwaysOnRec.onerror = null; _alwaysOnRec.abort(); } catch(e) {}
    _alwaysOnRec = null;
  }
...
async function startConvoMode() {
  ...
  // Multi-party speaker-ID: disabled for now. It opens its own separate
  // getUserMedia mic stream that runs the entire session, on top of the
  // recognizer's own stream (and a third opened during AI speech for
  // barge-in detection). Multiple concurrent mic consumers is a likely
  // cause of dropped/missed speech on some phones — the OS can duck or
  // degrade one of the streams. Speaker-ID is a nice-to-have (approximate
  // voice clustering, not real identification); disabling it removes one
  // source of mic contention. Safe to re-enable (uncomment resetRoster()
  // and _startSpeakerTap()) once reliability isn't the top priority.
  // resetRoster();
  // _startSpeakerTap();
