...
const VAD_WARMUP_MS       = 500;   // ignore this long after the watch opens — without headphones, the AI's
                                    // own speaker output bleeds into the mic right as playback ramps up, and
                                    // the browser's echo cancellation hasn't adapted yet. Skipping this brief
                                    // window is what stops the AI from "hearing itself" and self-interrupting
                                    // after just its first word.
...
export async function startBargeInWatch(onBargeIn) {
  ...
  try {
    _vadStream = await navigator.mediaDevices.getUserMedia({
      audio: {
        echoCancellation: true,  // critical without headphones — cancels the AI's own speaker output
  ...
  _vadAboveSince = null;
  const _vadStartedAt = performance.now();
  ...
  const poll = () => {
    ...
    if (performance.now() - _vadStartedAt < VAD_WARMUP_MS) {
      // Still in the warm-up window — keep polling (so the sustain clock
      // isn't stale once warm-up ends) but never let it count toward a trigger.
      _vadAboveSince = null;
